...
import { getAlbumData } from '@/api/collected';
...
const data: RootObject = reactive({
  ...
  albumList: [],
  list: computed(() => {
    return data.albumList.filter((item: any, index: number) => {
...
<template>
  <div class="albumContainer">
    <div class="head">
      收藏的专辑<span>({{ data.total }})</span>
      <el-input v-model="data.keyWord" class="w-50 m-2" size="small" placeholder="搜索收藏专辑"
    ...
    <div v-if="data.list.length == 0 && data.keyWord != ''" class="no-data">
      <span>未能找到与<a class="keyword">"{{ data.keyWord }}"</a>相关的任何专辑</span>
    </div>
  </div>
</template>
