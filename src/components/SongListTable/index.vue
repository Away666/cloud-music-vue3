...
const emits = defineEmits(['dbClickRow', 'deleteLikeMusic', 'deleteMusic'])
...
const likeIt = (id: any, index: number, item: any) => {
  ...
  // 如果是在我喜欢的音乐取消喜欢
  let isMyLikeMusic = store.state.aside.createdMusicList.findIndex((item: any, index: number) => {
    if (index === 0) {
...
// 在我喜欢的音乐取消喜欢
const deleteLikeMusic = () => {
  let timestamp = Date.parse(new Date().toString())
  like(data.currentId, false, timestamp).then((res) => {
    if (res.data.code === 200) {
      store.dispatch('songList/updateLikeMusicList', { type: 'del', id: data.currentId })
      emits('deleteLikeMusic', data.currentIndex)
    }
  })
...
const deleteMusic = () => {
  addOrDelSongApi('del', route.params.id, data.currentMusic.id).then((res) => {
    if (res.data.body.code === 200) {
      data.deleteDialog = false
      let index = props.songs.findIndex((item: any) => {
        return item.id == data.currentMusic.id
      })
      emits('deleteMusic', index)
    }
  })
}
...
onMounted(() => {
  ...
  document.getElementsByTagName('body')[0].addEventListener('keydown', function (e) {
    if (data.dialogVisible && e.keyCode == 13) {
      deleteLikeMusic()
    }
  });
})
...
watch(() => props.songs, (current, last) => {
  getLikeMusicHandle()
})
...
<template>
  <div>
    <div v-if="props.songs.length != 0" class="songs-container">
      <el-table :data="props.songs" size="mini" style="width: 100%" @row-dblclick="dbClickRow" highlight-current-row
        ...
        <el-table-column label="操作" prop="id" width="35" class-name="like" align="center">
          <template #default="scope">
            ...
            <i v-else class="iconfont icon-aixin2" @click="likeIt(scope.row.id, scope.$index, scope.row)"
              title="喜欢"></i>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="音乐标题" min-width="250">
          <template #default="{ row, $index }">
            <div class="table-name-col">
              <span class="name" :class="$index == props.playingIndex ? 'currentRow' : ''">{{ row.name }}</span>
              <span class="sub-head" v-if="row.alia">
                <span v-for="(item, index) in row.alia" :key="index">
                  <span v-if="index === 0">({{ item }})</span>
                </span>
              </span>
              <span class="no-resource" v-if="row.noCopyrightRcmd">无音源</span>
              <span class="tip" v-if="row.noCopyrightRcmd">{{ row.noCopyrightRcmd.typeDesc }}</span>
              <span class="vip" v-if="row.fee == 1 && !row.noCopyrightRcmd">VIP</span>
              <span class="vip" v-if="row.fee == 1 && !row.noCopyrightRcmd">试听</span>
              <span class="Hi-Res" v-if="row.hr && !row.noCopyrightRcmd">Hi-Res</span>
              <span class="Hi-Res" v-if="row.sq && !row.hr && !row.noCopyrightRcmd">SQ</span>
              <span class="mv" v-if="row.mv && !row.noCopyrightRcmd" @click="toMvDetail(row.mv)">MV
                <p class="symbol">▶</p>
              </span>
            </div>
          </template>
    ...
    <div v-else-if="props.songs.length == 0 && props.keyWord != ''" class="no-data">
      <span>未能找到与<a class="keyword">"{{ props.keyWord }}"</a>相关的任何音乐</span>
    </div>
    <el-dialog v-model="data.dialogVisible" width="30%" center :modal="false" :destroy-on-close="true"
      custom-class="delete-like-music-dialog" :close-on-click-modal="false">
      <span>确定将选中歌曲从我喜欢的音乐中删除？</span>
      <template #footer>
        <span class="dialog-footer">
          <el-button type="primary" @click="deleteLikeMusic">确定</el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="data.deleteDialog" width="30%" center :modal="false" :destroy-on-close="true"
      custom-class="delete-like-music-dialog" :close-on-click-modal="false">
      <span>确定将选中的歌曲从歌单中删除？</span>
      <template #footer>
        <span class="dialog-footer">
          <el-button type="primary" @click="deleteMusic">确定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
