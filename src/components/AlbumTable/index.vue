...
const emits = defineEmits(['dbClickRow', 'deleteLikeMusic'])
      ...
      <el-table :data="props.songs" size="mini" style="width: 100%" @row-dblclick="dbClickRow" highlight-current-row
        ...
        <el-table-column label="" width="50" type="index" :index="handleIndex" align="right">
          <template #default="scope">
            <span
              v-if="scope.$index == props.playingIndex && store.state.songList.musicId == scope.row.id && store.state.songList.isPlaying"
              class="currentRow"><i class="iconfont icon-icon-yinliangzhong"></i></span>
            <span
              v-else-if="scope.$index == props.playingIndex && store.state.songList.musicId == scope.row.id && store.state.songList.isPlaying == false"
        ...
        <el-table-column prop="name" label="音乐标题" min-width="230">
          <template #default="{ row, $index }">
            <div class="table-name-col">
              <span class="name"
                :class="{ 'currentRow': $index == props.playingIndex && store.state.songList.musicId == row.id, 'not-online': row.dt === 0 }">{{
                row.name
                }}</span>
        ...
        <el-table-column prop="al.name" label="专辑" min-width="100"><template #default="{ row }">
        ...
        <el-table-column prop="pop" label="热度" max-width="150">
          <template #default="{ row }">
            <el-progress :percentage="row.pop" :show-text="false" color="rgb(206, 206, 206)">
            </el-progress>
          </template>
        </el-table-column>
      </el-table>
