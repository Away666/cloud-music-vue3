...
const dbClickRow = (row: any, col: any, e: any) => {
  if (row.dt === 0) {
    ElMessage({
      type: 'warning',
      message: '歌曲即将上线，敬请期待！'
    ...
    console.log(row.noCopyrightRcmd);
    ElMessage({
      type: 'error',
    ...
    emits('dbClickRow', row.id, props.currentAlbumIndex)
  }
  else {
    emits('dbClickRow', row.id, props.currentAlbumIndex)
  }
      ...
      <el-table-column prop="name" label="音乐标题" min-width="250">
        <template #default="scope">
          <div class="table-name-col">
            <span class="name"
              :class="{ 'currentRow': scope.$index == props.playingIndex, 'not-online': scope.row.dt === 0 }">{{
              scope.row.name
              }}</span>
